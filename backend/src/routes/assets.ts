  ...
  try {
    ...
    const asset = await prisma.asset.create({
      data: {
        ...
        specifications,
        documents,
        imageUrls: JSON.stringify(imageUrls || []) as any,
        fileUrls: JSON.stringify(fileUrls || []) as any,
        notes,
        organizationId,
        createdById
      },
...
router.put('/:id', async (req, res) => {
  try {
    ...
    if (updateData.installationDate) updateData.installationDate = new Date(updateData.installationDate);
...
router.get('/:id/maintenance-history', async (req, res) => {
  try {
    ...
    const maintenanceHistory = await prisma.workOrder.findMany({
      where: {
        assetId: id,
